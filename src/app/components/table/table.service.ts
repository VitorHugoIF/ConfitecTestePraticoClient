...
import { SortDirection } from './table.directive';
...
export class TableService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 2. filter
    tables = tables?.filter(table => this.matches(table, searchTerm));
  ...
  sort(dadosTabela: any[], column: string, direction: string): any[] {
    if (direction === '') {
      return dadosTabela;
    } else {
  ...
  private matches(tables: any, term: string): any {
    if (this.matchFunction) {
      return this.matchFunction(tables, term)
    }
    return true;
  }
  setTableData(tableData: any[]) { this.tableData = tableData; }
  onMatch(callback: Function) { this.matchFunction = callback; }
